"""
Serviço responsável por fazer o web scraping dos dados de importação da Embrapa
"""
import requests
...
class ServicoImportacao:
    ...
    def coletarDadosImportacao(self, ano: int, opcao: str = None, subopcao: str = None) -> pd.DataFrame:
        """
        Faz o web scraping dos dados de importação da Embrapa para o ano específicado
        
        Args:
            ano (int): O ano para o qual se deseja obter os dados
            opcao (str): Opção do relatório (padrão: 'opt_05')
            subopcao (str): Subopção do relatório (padrão: 'subopt_03')
            
        Returns:
            pd.DataFrame: DataFrame com os dados de importação de vinho
        """
        ...
        for tabela in tabelas:
            ...
            for linha in linhas:
                ...
                if len(colunas) >= 3:  # Importação tem 3 colunas (País, Quantidade, Valor)
