...
def obter_dados_processamento():
    ...
    opcao = request.args.get('opcao', default=Configuracao.OPCAO_PROCESSAMENTO, type=str)
    subopcao = request.args.get('subopcao', default=Configuracao.SUBOPCAO_PROCESSAMENTO_PADRAO, type=str)
...
@api_blueprint.route('/embrapa_comercializacao', methods=['GET'])
def obter_dados_comercializacao():
    """
    Endpoint para obter os dados de comercialização da Embrapa
    ...
    ano = request.args.get('ano', default=Configuracao.ANO_PADRAO, type=int)
    formato = request.args.get('formato', default='padrao', type=str)
    opcao = request.args.get('opcao', default=Configuracao.OPCAO_COMERCIALIZACAO, type=str)
